...
def assert_file_is_accessible(url: str):
    ...
    """
    logger.info(f"Check file is accessible")
...
@allure.step("Check get file with incorrect file is response")
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    ...
    :raises AssertionError: Если фактический ответ не соответствует ожидаемому.
    """
    logger.info(f"Check get file with incorrect file is response")

    expected = ValidationErrorResponseSchema(
        details=[
            ValidationErrorSchema(
                type="uuid_parsing",  # Тип ошибки, связанной с невалидным UUID.
                input="incorrect-file-id",  # Некорректный uuid.
                context={
